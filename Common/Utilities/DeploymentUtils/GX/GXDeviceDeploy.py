...
class GXDeviceDeploy(object):
    ...
    def get_currently_running_gx_version(self, current_process=None):
        """
        :return: version number i.e. 9.0.1.6666
        """
        if not current_process:
            current_process = self.get_current_running_gx_process()

        if current_process:
            return re_search("corero/defense-V(\d.\d*.\d.\d+)", current_process).group(1)
        return None

    @retry(exceptions=DeploymentException, delay=5, tries=5)
    def kick_start_gx(self, process_name_to_start):
        currently_running_process = self.get_current_running_gx_process()

        currently_running_ntd = self.get_currently_running_gx_version(currently_running_process)
        ...
        currently_running_process = self.get_current_running_gx_process()
        if process_name_to_start not in currently_running_process:
            raise DeploymentException('Expected GX version not running.')

        PrintMessage('Installed GX version: {0}'.format(process_name_to_start))

    def _install_from_tar(self, device_to_deploy, ntd_release_version):
        ntd_release_version_re = 'defense-V(\d.\d*.\d.\d+_PGO)'.format(ntd_release_version)
        tar_file = self._copy_tar_file(device_to_deploy.version.build, ntd_release_version)
        FileTools.is_remote_file_present_in_current_folder(self.shell, tar_file)

        self.shell.send_cmd('tar -xf {0}'.format(tar_file), delay=10)
        ...
        self.kick_start_gx(process_name_to_start)

        return process_name_to_start

    def verify_installed_version(self, installed_version):
        current_running = self.get_current_running_gx_process()

        assert installed_version in current_running, "actual: {0}, expected: {1}".format(current_running,
                                                                                         installed_version)

    def deploy(self, device_to_deploy):
        assert device_to_deploy.version.build is not None, "Build number must be supplied for gx deployment"

        self.log_as_support(device_to_deploy.name)
        current_running_process = self.get_current_running_gx_process()
        current_running_version = self.get_currently_running_gx_version(current_running_process)
